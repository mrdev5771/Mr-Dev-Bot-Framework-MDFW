...
module.exports.run = async function ({ api, event, args }) {
  ...
  const cache = path.join(__dirname, "cache/canvas/gandhi");
  await fs.ensureDir(cache);
  const out = path.join(cache, `aftab_${Date.now()}.png`);
  const templateUrl = "https://i.postimg.cc/XYVd25pQ/Ssd2lmc.png";

  // layout config (tune to template)
  const area = { x: 350, y: 180, w: 739, h: 415 };
  ...
  try {
    ...
    console.error("[gandhi] error:", err && (err.stack || err.message || err));
    await api.sendMessage(
